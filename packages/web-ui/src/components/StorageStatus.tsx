...
export function StorageStatus() {
  ...
  useEffect(() => {
    const calculateSize = () => {
      ...
      const captionSize = captionFile
        ? Math.round((JSON.stringify(captionFile).length / 1024) * 100) / 100
        : 0; // KB, not MB
  ...
  const handleClearStorage = async () => {
    if (
      !confirm(
        'Clear all stored data? This will remove your saved video and captions.'
      )
    ) {
      return;
    }
    ...
    <div className="bg-gray-50 rounded-lg border p-3 text-sm">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2 text-gray-600">
          ...
          <span>
            {storageSize > 0
              ? `Caption data: ${storageSize.toFixed(1)}KB`
              : 'No captions to save'}
          </span>
          {captionFile && (
        ...
        <div className="flex items-center space-x-2">
          {storageSize > 50 && (
            <span className="text-xs text-orange-600">Large storage usage</span>
          )}
      ...
      {(video.fileName || captionFile) && (
        <div className="mt-2 text-xs text-gray-500">
          ...
          {captionFile && (
            <span>
              {video.fileName ? ' • ' : ''}
              {captionFile.segments.length} saved caption segments
            </span>
          )}
        </div>
      )}
